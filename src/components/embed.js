...
const playEmbed = (queue, track, currIndex) => new EmbedBuilder()
    .setAuthor({
        name: "🔥 ĐANG PHÁT 🔥",
    ...
    .addFields({ name: '💽 Danh sách', value: `\`${currIndex + 1}/${queue.length}\``, inline: true })
    .setTimestamp()
    .setFooter({ text: "_Developed by Binhkio_" })

const endedEmbed = (queue, track) => new EmbedBuilder()
    .setAuthor({
        name: "🔥 ĐÃ PHÁT 🔥",
        iconURL: track?.user.avatarURL({ extension: 'png' }) || undefined,
    })
